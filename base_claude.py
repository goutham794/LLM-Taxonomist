...
class LLM_Agent(ABC, Generic[InputType, OutputType]):
    ...
    def _run(self, user_input: str, input_data: InputType) -> BaseModel:
        ...
        output_schema = self.response_base_model.model_json_schema()
        tools = [
            {
                "name": f"build_{self.__class__.__name__}_result",
                "description": f"build the {self.__class__.__name__} output object",
        ...
        while num_retries < self.max_retries:
            ...
            tools=tools,
            tool_choice={"type": "tool", "name": f"build_{self.__class__.__name__}_result"}
            )

            results = response.content[0].input

            if self._validate_response(input_data, self.response_base_model(**results)) is False:
                ...
                return self.response_base_model(**results)
...
class Categorizer_Agent(LLM_Agent[List[LeafTopic], Categorizer_Agent_Output]):
    def __init__(self, client: anthropic.Anthropic, system_instruction: str = None,
        ...
        if system_instruction is None:
            ...
            system_instruction = dedent(f"""\
                                ...
                                You are given a list of topics from the domain {domain}.
...
class CategoryHarmonizerAgent(LLM_Agent[HarmonizerInput, HarmonizerOutput]):
    def __init__(self, client: anthropic.Anthropic, system_instruction: str = None, response_base_model: BaseModel = HarmonizerOutput, domain: str = None):
        if system_instruction is None:
            assert domain is not None, "If system_instruction is not provided, domain must be provided."
            system_instruction = dedent(f"""\
...
class TaxonomyBuilder:
    def __init__(self, client: anthropic.Anthropic, domain: str):
        self.client = client
        self.domain = domain
